...
namespace Daigassou.Forms
{
    ...
    {
        private readonly Dictionary<string, HotKeyControl> keyBindings;
        ...
        {
            HotkeyUtils.GetInstance().ResetConfig();
            loadKeyconfig();
        }
        ...
        private void hotKeyControl4_HotKeyIsSet(object sender, HotKeyIsSetEventArgs e)
        {
            ...
            if (name != null)
            {
                ...
                if (s.UserModifier.HasFlag(Modifiers.Control)) finalKey |= Keys.Control;
                if (s.UserModifier.HasFlag(Modifiers.Alt)) finalKey |= Keys.Alt;
                if (s.UserModifier.HasFlag(Modifiers.Shift)) finalKey |= Keys.Shift;
                if (s.UserModifier.HasFlag(Modifiers.Win)) finalKey |= Keys.LWin;
